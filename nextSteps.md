...
- **FE:** Show the seed on the detail page; a button clones the config *with* the seed
...
## Epic 6 — Charts on the metrics dashboard

Metrics are currently plain aggregate numbers. Each chart is one slice; each reads data
that already exists via the `/detail` or `/visualisation` endpoints (add a small
aggregation only if needed). Use the `dataviz` skill for all of these.

### Slice 6.1 — Wait-time distribution histogram
- **Test (manual):** A congested run shows a right-skewed distribution; an uncongested one
  clusters near zero.

### Slice 6.2 — Throughput over time
- **Test (manual):** Completions-per-interval line tracks the configured rates.

### Slice 6.3 — Per-runway utilisation %
- **Test (manual):** With one runway closed for part of the run, its utilisation is
  visibly lower.
...
4. **Epic 4 → 5 → 6** (compare → sweep → charts) — the analysis story that makes this a
   real modelling tool.
