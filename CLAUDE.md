...
- [`.github/workflows/ci.yml`](.github/workflows/ci.yml) runs on every push/PR: a `backend` job (`pytest`, no external services needed — see below) and an independent `frontend` job (`npm run build` + `npm run lint`). No deployment pipeline exists beyond this — CI only.
- A [`docker-compose.yml`](docker-compose.yml) at the repo root brings up the **backend** side of local dev (Postgres, Redis, `runserver`/`web`, the `dramatiq` worker, and a `watchdog` service looping `check_stalled_simulations`, via [`backend/Dockerfile`](backend/Dockerfile)) as a single source of truth for "which process is running" — `docker compose up` migrates then starts `web`/`worker`/`watchdog`. It deliberately does **not** include the frontend; run that with `npm run dev` as usual, pointed at the dockerized backend's `http://localhost:8000`. It's an alternative to the manual `venv`/`runserver`/`rundramatiq` workflow below, not a supplement to it — don't run both against the same Postgres/Redis ports at once (the manual workflow's default ports 5432/6379 are what the compose file also binds to the host).
- `manage.py check_stalled_simulations` (no args) marks any `Simulation` stuck `Running` with a stale `last_heartbeat_at` (bumped periodically by `SimulationRunner`'s in-run watchdog) as `Error` — the mitigation for the exact "stray worker leaves a sim Running forever" failure mode this file documents incidents of above. Outside Docker Compose (which loops it automatically via the `watchdog` service), nothing invokes this on a schedule — run it manually, or wire up your own cron/Task Scheduler entry, for the manual dev workflow.
